...
public class JogRecordContentProvider extends ContentProvider {
    ...
    private static final String AUTHORITY = "com.fm.JogRecord.JogRecordContentProvider";
    ...
    public boolean onCreate() {
        mDpHelper = new DatabaseHelper(getContext());
        return false;
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        int uriType = uriMatcher.match(uri);
        SQLiteDatabase sqlDB = mDpHelper.getWritableDatabase();
        long id = 0;
        switch (uriType) {
            case JOGRECORD:
